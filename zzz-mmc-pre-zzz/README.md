...
#### `kube_ps.sh` - Kubernetes Prompt Helper
...
#### `kubens.sh` - Kubernetes Namespace Manager
...
#### `aws-waf.html` - AWS WAF & CloudFront Configuration Guide
...
2. **Make scripts executable:**
   ```bash
   chmod +x kubectx.sh kubens.sh
   ```

3. **Add to your shell configuration:**
   ```bash
   # For bash (.bashrc)
   source /path/to/zzz-mmc-pre-zzz/kube_ps.sh
...
4. **Optional: Add scripts to your PATH:**
   ```bash
   export PATH="$PATH:/path/to/zzz-mmc-pre-zzz"
   ```
...
./kubens.sh monitoring

# Quick context/namespace switching
./kubectx.sh -  # Previous context
./kubens.sh -   # Previous namespace
```

### Enhanced Shell Prompt

After setting up `kube_ps.sh`, your prompt will display:
```
...
- **kube_ps.sh**: Apache License 2.0 (Copyright 2025 Jon Mosco)
- **kubectx.sh**: Apache License 2.0 (Copyright 2017 Google Inc.)
- **kubens.sh**: Apache License 2.0 (Copyright 2017 Google Inc.)
- **aws-waf.html**: Custom documentation
